...
function obtenerLaborales($id_candidato)
{
	...
	$dataes = "nombre_empresa_candidato_";
	$queryz = "SELECT COUNT(*) AS CantidadLaborales FROM ad_bgc WHERE meta_key LIKE '%$dataes%' AND id_can='$id_candidato'";
	echo "$queryz<br>";
	$sql = mysqli_query($conn, $queryz);
	$r = mysqli_fetch_assoc($sql);
	$cantidad = $r['CantidadLaborales'];

	$dataes2 = "nombre_empresa_analista_";
	$queryz2 = "SELECT COUNT(*) AS CantidadLaboralesAnalyst FROM ad_bgc WHERE meta_key LIKE '%$dataes2%' AND id_can='$id_candidato'";
	echo "$queryz2<br>";
	$sql2 = mysqli_query($conn, $queryz2);
	$r2 = mysqli_fetch_assoc($sql2);
	$cantidadAnalyst = $r2['CantidadLaboralesAnalyst'];
	echo $cantidad;
	if($cantidad != 0)
	{
		// Si existen laborales
		for($i=1; $i<=$cantidad; $i++)
		{
			// Comienza lo chingon!
			$query = "SELECT * FROM ad_bgc WHERE id_can='$id' AND meta_key IN('nombre_empresa_candidato_$i','direccion_empresa_candidato_$i','fechaIn_laboral_candidato_$i','fechaOut_laboral_candidato_$i','numero_empresa_candidato_$i','puesto_inicial_candidato_$i','puesto_final_candidato_$i','salario_inicial_candidato_$i','salario_final_candidato_$i','jefeInmediato_candidato_$i','gerentePos_candidato_$i','separacion_candidato_$i','nombre_empresa_analista_$i','direccion_empresa_analista_$i','fechaIn_laboral_analista_$i','fechaOut_laboral_analista_$i','numero_empresa_analista_$i','puesto_inicial_analista_$i','puesto_final_analista_$i','salario_inicial_analista_$i','salario_final_analista_$i','jefeInmediato_analista_$i','gerentePos_analista_$i','separacion_analista_$i')";
			echo $query."<br><br>";
			$sa = mysqli_query($conn, $query);

			$campos2 = array("nombre_empresa_candidato_$i","direccion_empresa_candidato_$i","fechaIn_laboral_candidato_$i","fechaOut_laboral_candidato_$i","numero_empresa_candidato_$i","puesto_inicial_candidato_$i","puesto_final_candidato_$i","salario_inicial_candidato_$i","salario_final_candidato_$i","jefeInmediato_candidato_$i","gerentePos_candidato_$i","separacion_candidato_$i");
			$titulos = array("Nombre de Empresa $i","Dirección de la Empresa $i","Fecha de Ingreso Laboral $i","Fecha de Salida Laboral $i","Teléfono de la Empresa $i","Puesto Inicial $i","Puesto Final $i","Salario Inicial $i","Salario Final $i","Jefe Inmediato $i","Puesto del Jefe Inmediato $i","Motivo de Separación $i");
			$p = 0;
			$meta_key = array();
			$meta_value = array();
			$textos_campos = array();
			while($row = mysqli_fetch_assoc($sa))
			{
				...
				$meta_value[] = ucfirst(strtolower($row['meta_value']));
			}
		}
	} else {
		// No existen laborales
	}
...
function obtenerViviendasAnteriores($id_candidato)
{
	...
	if(mysqli_num_rows($sql) != 0)
	{
		...
		for($i=1; $i<=$lomb[3]; $i++)
		{
			...
			$texto = $dataPerson2[4].", ".$dataPerson2[0]." #".$dataPerson2[1].", Col. ".$dataPerson2[2].", ".$dataPerson2[3];
			// Imprime la variable
